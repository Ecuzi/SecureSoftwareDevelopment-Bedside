...
public class LoginController implements Initializable {
    ...
    // When in authenticator this determines with scene will pop up when it is authenticated (Employee Side, Guest Side)
    ...
    public void loginButtonOnAction(ActionEvent event) throws MessagingException, IOException {
        ...
        if (usernameTextField.getText().isBlank() == false && enterPasswordField.getText().isBlank() == false) {
            ...
            // Checks to see if the credentials are in the system
            if(checkCredentials()) {
            	validateLogin();
            }
            // Else returns error
    ...
    private boolean checkCredentials() {
		try {
				...
				pass = enterPasswordField.getText();
				int x = ServerConnection.accessLevel(email, pass);
				if(x > 0) {
					AC.setEmployeeOrGuest(1);
					return true;
				}
		}
		catch(Exception e) {
    ...
    public void hyperlinkOnAction(ActionEvent event) throws IOException {
        Parent guestViewParent = FXMLLoader.load(getClass().getResource("GuestLogin.fxml"));
        Scene guestScene = new Scene(guestViewParent);
		guestScene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
    ...
    public void validateLogin() throws MessagingException, IOException {
        AuthenticationController AC = new AuthenticationController();
        AC.setCode();
        String to = usernameTextField.getText();
        setEmail(to);
